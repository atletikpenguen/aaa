...
class BollingerGridStrategy(BaseStrategy):
    ...
    def _should_close_cycle(self, remaining_quantity: float, initial_usdt: float, current_price: float) -> bool:
        ...
        try:
            ...
            if current_price <= 0 or initial_usdt <= 0:
                return False
            ...
            self._debug_log("", f"⚠️ Döngü kapanma kontrolü hatası: {e}", "WARNING")
            return False
    ...
    def _get_bollinger_bands(self, ohlcv_data: List, period: int, std_dev: float) -> Tuple[BollingerBands, BollingerBands]:
        ...
        try:
            ...
            # Fiyat listesini hazırla - güvenli dönüşüm
            closes = []
            for candle in ohlcv_data:
                try:
                    close_price = float(candle[4])
                    if close_price > 0:  # Geçerli fiyat kontrolü
            ...
            # Son ve önceki değerler - güvenli erişim
            current_bands = BollingerBands(
                upper=bb_result['upper'][-1] if bb_result['upper'] else 0.0,
                middle=bb_result['middle'][-1] if bb_result['middle'] else 0.0,
                lower=bb_result['lower'][-1] if bb_result['lower'] else 0.0
            )
            
            prev_bands = BollingerBands(
                upper=bb_result['upper'][-2] if len(bb_result['upper']) >= 2 else 0.0,
                middle=bb_result['middle'][-2] if len(bb_result['middle']) >= 2 else 0.0,
                lower=bb_result['lower'][-2] if len(bb_result['lower']) >= 2 else 0.0
            )
            
            return current_bands, prev_bands
            
        except Exception as e:
            self._debug_log("", f"⚠️ Bollinger Bands hesaplama hatası: {e}", "ERROR")
            default_bands = BollingerBands(upper=0.0, middle=0.0, lower=0.0)
            return default_bands, default_bands
    ...
    async def calculate_signal(
        ...
        try:
            ...
            if prev_price <= 0:
                ...
                self._debug_log(strategy.id, f"⚠️ Önceki fiyat geçersiz, mevcut fiyat kullanılıyor: {prev_price}")
            ...
            if prev_price > 0:
                try:
                    ...
                    price_change_pct = 0.0
                    self._debug_log(strategy.id, "⚠️ Fiyat değişim yüzdesi hesaplanamadı, 0 olarak ayarlandı")
    ...
    async def process_fill(
        ...
        try:
            ...
            if not trade or trade.quantity <= 0 or trade.price <= 0:
                self._debug_log(strategy.id, f"⚠️ Geçersiz trade verisi: {trade}", "WARNING")
                return {}
        ...
        if trade.side == OrderSide.BUY:
            ...
            self._debug_log(strategy.id, f"❌ Process fill genel hatası: {e}", "ERROR")
            return {}
